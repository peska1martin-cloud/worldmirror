...
def vytahni(text, klic):
    if not text: return ""
    # Najde klíč a očistí ho od hvězdiček a odrážek
...
if st.session_state.view == 'map':
    ...
    if st.button("🚀 Spustit hloubkový sken světa"):
        with st.spinner("Skenuji světové zdroje..."):
            model = ziskej_funkcni_model()
            clanky, text_ai = stahni_zpravy()
            if model and clanky:
                prompt = f"""
                Jsi elitní analytik. Z těchto 50 zpráv vyber přesně 10 nejdůležitějších témat.
                Každé téma odděl přesně třemi pomlčkami '---'.
                U každého tématu uveď:
                TÉMA: [Název]
                KATEGORIE: [Válka/Ekonomika/Politika/Technologie]
                LAT: [Zeměpisná šířka - jen číslo]
                LON: [Zeměpisná délka - jen číslo]
                BLESKOVKA: [Krátká věta]
                FAKTA: [Popis]
                USA: [Postoj USA]
                EU: [Postoj Evropy]
                ...
                ZDROJE_ID: [ID čísel článků oddělená čárkou, např: 0, 5, 12]
                ---
                ZDROJE ČLÁNKŮ S ID:
                {text_ai}
                """
    ...
    if historie:
        report = historie[0]
        # FILTRACE: Vezmeme jen kousky textu, které skutečně obsahují "TÉMA", a omezíme na 10
        ...
        for i, t_data in enumerate(seznam_temat):
            ...
            try:
                # Kontrola, zda jsou souřadnice čísla
                folium.CircleMarker(
        ...
        st.subheader(f"📌 Aktuálních {len(seznam_temat)} témat")
                ...
                if idx < len(seznam_temat):
                    ...
                    with cols[j]:
                        with st.container(border=True):
                            ...
                            if st.button(f"🔎 Otevřít detail", key=f"btn_{idx}"):
    ...
    except: st.write("Chyba při načítání odkazů.")
